...
const toggleSubscription = asyncHandler(async (req, res) => {
    ...
    const unsubscribe = await Subscriptions.findByIdAndDelete(subscribed._id)
    if (!unsubscribe) {
...
const getSubscribedChannel = asyncHandler(async (req, res) => {
    ...
    if (!isValidObjectId(channelId)) {
        throw new apiError(404, "Invalid subcriber ID")
    }
    const channelList = await Subscriptions.aggregate([
        {
            $match: {
